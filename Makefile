...
backup-db:
	...
	@mkdir -p backups
	@uv run python -c "from django.conf import settings; import os; os.environ.setdefault('DJANGO_SETTINGS_MODULE', 'config.settings'); import django; django.setup(); db = settings.DATABASES['default']; print(f\"pg_dump -h {db['HOST']} -p {db['PORT']} -U {db['USER']} -d {db['NAME']} > backups/backup_$$(date +%Y%m%d_%H%M%S).sql\")" | sh
...
restore-db:
	...
	uv run python -c "from django.conf import settings; import os; os.environ.setdefault('DJANGO_SETTINGS_MODULE', 'config.settings'); import django; django.setup(); db = settings.DATABASES['default']; print(f\"psql -h {db['HOST']} -p {db['PORT']} -U {db['USER']} -d {db['NAME']} < backups/$$backup_file\")" | sh && \
...
import-denoms:
	uv run python manage.py import_denominations static-data/denominations.csv --clear-existing
